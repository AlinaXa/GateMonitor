...
Item {
    ...
    Column { anchors.left: parent.left; anchors.right: parent.right; anchors.top: parent.top; anchors.bottom: nav.top; anchors.margins: 16; anchors.topMargin: 42; spacing: 12
        ...
        Repeater { model: [{label:"Profilul meu",detail:"Date personale și preferințe",action:"profile"},{label:"Statistici",detail:"KPI, activitate și predicții",action:"statistics"},{label:"Istoric și audit",detail:"Căutare, calendar și modificări",action:"history"},{label:"Schimbă locația",detail:"Selectează alt depou",action:"locations"},{label:"Poarta curentă",detail:"Deschide poarta selectată",action:"gate"}]
            delegate: Rectangle { required property var modelData; width: root.width - 32; height: 68; radius: 12; color: mouse.containsMouse ? App.Theme.cardHover : App.Theme.card; border.color: App.Theme.cardBorder
                ...
                MouseArea { id: mouse; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: { if (modelData.action === "locations") root.openLocations(); else if (modelData.action === "gate") root.openGate(); else if (modelData.action === "profile") root.openProfile(); else if (modelData.action === "history") root.openHistory(); else if (modelData.action === "statistics") root.openStatistics() } }
            }
        }
        ...
        Rectangle { width: parent.width; height: 46; radius: 12; color: "transparent"; border.color: App.Theme.red
            Text { anchors.centerIn: parent; text: "Deconectare"; color: App.Theme.red; font.pixelSize: 11; font.weight: Font.DemiBold }
            MouseArea { anchors.fill: parent; onClicked: root.signOutRequested() }
        }
        Text { text: "Gate Monitor · versiunea 1.5"; color: App.Theme.secondaryText; font.pixelSize: 10; anchors.horizontalCenter: parent.horizontalCenter }
    }
